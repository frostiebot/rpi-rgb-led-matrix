...
namespace rgb_matrix {
  int DrawText(Canvas *c, const Font &font, int x, int y, const Color &color, const Color *background_color, const char *utf8_text, int kerning_offset) {
    const int start_x = x;
    while (*utf8_text) {
  ...
  // of months.
  int DrawText(Canvas *c, const Font &font, int x, int y, const Color &color, const Color *background_color, const char *utf8_text) {
    return DrawText(c, font, x, y, color, background_color, utf8_text, 0);
  }

  int VerticalDrawText(Canvas *c, const Font &font, int x, int y, const Color &color, const Color *background_color, const char *utf8_text, int extra_spacing) {
    const int start_y = y;
    while (*utf8_text) {
      const uint32_t cp = utf8_next_codepoint(utf8_text);
  ...
  void DrawFilledCircle(Canvas *c, int x0, int y0, int radius, const Color &color) {
    ...
    while (y <= x) {
      DrawLine(c, x0 + x, y0 + y, x0 - x, y0 + y, color);
      DrawLine(c, x0 + y, y0 + x, x0 - y, y0 + x, color);
      DrawLine(c, x0 - x, y0 - y, x0 + x, y0 - y, color);
      DrawLine(c, x0 - y, y0 - x, x0 + y, y0 - x, color);
      y++;
      if (radiusError<0) {
  ...
  void DrawRectangle(Canvas *c, int x0, int y0, int x1, int y1, const Color &color) {
    DrawLine(c, x0, y0, x0, y1, color);  // Top-Left to Bottom-Left
    DrawLine(c, x0, y1, x1, y1, color);  // Bottom-Left to Bottom-Right
    DrawLine(c, x1, y1, x1, y0, color);  // Bottom-Right to Top-Right
    DrawLine(c, x1, y0, x0, y0, color);  // Top-Right to Top-Left
  }

  void DrawFilledRectangle(Canvas *c, int x0, int y0, int x1, int y1, const Color &color) {
    int y = std::min(y0, y1), dy = std::max(y0, y1) + 1;
    while (y < dy) {
      DrawLine(c, x0, y, x1, y, color);  // Row-by-Row
      y++;
    }
  }
